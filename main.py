...
def detect_wake_words():
    ...
    speech = LiveSpeech(
        hmm=os.path.join(
            r'/Users/soumya/Desktop/Projects/JARVIS.S/.venv/lib/python3.9/site-packages/pocketsphinx/model/en-us', 'en-us'),
        dic=os.path.join(
            r'/Users/soumya/Desktop/Projects/JARVIS.S/.venv/lib/python3.9/site-packages/pocketsphinx/model/en-us', 'cmudict-en-us.dict'),
        kws=kws_list_path
    )
    print("Wake me")
...
def market_data():
    ...
    if len(sensex_data) < 2 or len(live_data_sensex) == 0:
        return "Insufficient data for Sensex."
    if len(nifty_data) < 2 or len(live_data_nifty) == 0:
